...
function _writeBufferToDisk(buff, withPath, cb) {
  var fullPath = withPath || path.join(tmpDir, "textract_file_" + _genRandom());
...
function fromBufferWithMime(type, bufferContent, options, withPath, cb) {
  if (
    typeof type === "string" &&
    bufferContent &&
    ...
    _writeBufferToDisk(bufferContent, withPath, function (newPath) {
